function checkBoxActive(_onClick) : widget() constructor {
	...
	static draw = function(_x, _y, _value, _m, _w, _h) {
		...
		var pd = 4 * THEME_SCALE;
		
		draw_sprite_stretched_ext(spr, _value, bx - pd, by - pd, bw + pd*2, bh + pd*2);
		
		if(hover && point_in_rectangle(_m[0], _m[1], bx, by, bx + bw, by + bh)) {
			draw_sprite_stretched_add(spr, _value, bx - pd, by - pd, bw + pd*2, bh + pd*2, COLORS._main_icon_dark);
		...
		draw_set_text(f_p3, fa_center, fa_center, _value? COLORS._main_value_positive : COLORS._main_value_negative);
